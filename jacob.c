...
//     t = clock(); 
	...
	for (int i = 0; i < P-1; ++i)
	{
		...
		if (returnstatus[i] == -1) {
	      printf(1,"Unable to create pipe %d \n", i);
	      return 1;
	    }

	    returnstatus2[i] = pipe(pipeup[i]);
		if (returnstatus2[i] == -1) {
	      printf(1,"Unable to create pipe2 %d \n", i);
	      return 1;
	    }
...
// my code
	...
	{	
		...
		for(;;){
				...
				int intd = diff*1000;
				if(k ==0)printf(1,"diff before is %d\n", intd);
				for(i =start ; i < end; i++){
					for(j =1 ; j < N-1; j++){
